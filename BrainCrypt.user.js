...
// Credits to the original author of this project: https://gitlab.com/An0

(function() {

  'use strict';
  
  const BlacklistUrl = "https://raw.githubusercontent.com/Braincell-Industries/BrainCrypt/main/blacklist.txt";
  ...
  var Discord;
  var Utils = {
      Log: (message) => { console.log(`%c[BrainCrypt] %c${message}`, `color:${BaseColor};font-weight:bold`, "") },
      Warn: (message) => { console.warn(`%c[BrainCrypt] %c${message}`, `color:${BaseColor};font-weight:bold`, "") },
      Error: (message) => { console.error(`%c[BrainCrypt] %c${message}`, `color:${BaseColor};font-weight:bold`, "") },
      Webpack: function() {
  ...
  function Init(final)
  {
      ...
      Object.assign(Utils, {
          ...
          LoadDb: function(callback, failCallback, reload) { (async () => {
              if(!reload) DataBase = await this.StorageLoad('BrainCrypt');
              if(DataBase != null) {
                  Cache = { keys: {} };
          ...
          SaveDb: async function() {
              ...
              await this.StorageSave('BrainCrypt', DataBase);
          },
          saveDbTimeout: null,
          FastSaveDb: function() {
              this.dbChanged = true;
          ...
          DownloadDb: async function(uncompressed) {
              ...
              this.DownloadBlob(uncompressed ? "BrainCrypt.json" : "BrainCrypt.dat", new Blob([buffer]));
          },
  ...
  async function handleUpdate(event) {
      ...
      if(message.content == null && message.embeds != null && message.embeds.length === 1) {
          ...
          if(embed.footer != null && (embed.footer.text === "BrainCrypt" || embed.footer.text === "Very Cryptic, Isn't it...")) {
  ...
  const messageRegex = /^([⠀-⣿]{16,}) `(?:BrainCrypt|Very Cryptic, Isn't it...)`$/;
  const systemMessageRegex = /^```(?:\w*\n)?-----SYSTEM MESSAGE-----\n?```\s*(.*?)\s*```(?:\w*\n)?(?:Very Cryptic, Isn't it...|BrainCrypt)\n?```$/s;
  ...
  async function decryptMessage(message, payload, ignoreAttachments) {
      ...
      if(payloadBuffer.byteLength === 16) {
          if(!differentKey) message.content = "<:ENC:1131990423359991879>⁣"; //invisible separator at the end to make the emoji smaller
  ...
  async function processEmbeds(message, ignoreAttachments) {
      ...
      let embed = message.embeds[0];
      if(embed.footer == null || (embed.footer.text !== "BrainCrypt" && embed.footer.text !== "Very Cryptic, Isn't it...")) return;
  ...
  async function handleSend(channelId, message, forceSimple) {
      ...
      if(forceSimple || Cache.channelBlacklist === 2 || (channel.type === 0 && !Utils.Can(EMBED_LINKS_CHECK, Discord.getCurrentUser(), channel))) {
         message.content = payload + " `Very Cryptic, Isn't it...`";
      }
      else {
          message.content = "";
          message.embed = {
              ...
              author: {
                  ...
                  url: "http://gitlab.com/An0/BrainCrypt"
              },
              description: payload,
              footer: {
                  text: "Very Cryptic, Isn't it...",
  ...
  Utils.Log("Injected\nEncryption stronger than Louis Cyr.");
